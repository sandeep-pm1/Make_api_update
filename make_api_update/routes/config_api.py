...
@router.get("/boards/subprocesses/{brands}/{opus}/{sites}/{process}?boardName=")
async def subprocess(brands: str, opus: str, sites: str, process: str, boardName: str = 'make'):
...
@router.get("/boards/subprocesses/{brands}/{opus}/{sites}/{process}")
async def subprocess(brands: str, opus: str, sites: str, process: str, boardName: str = "make"):
    validate_param_result =  validate_params(brands, opus, sites, process)
    if validate_param_result != False:
        response_data = {"subprocesses": dict_sub[validate_param_result.upper()]}
